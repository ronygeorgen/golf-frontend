...
function BookingManagement() {
    ...
    useEffect(() => {
        dispatch(getBookings({ filter, dateRange, page, bookingType, search: searchQuery }));
    }, [dispatch, filter, dateRange, page, bookingType, searchQuery]);
    ...
    const handleDateFilterChange = () => {
        setPage(1);
        dispatch(getBookings({ filter, dateRange, page: 1, bookingType, search: searchQuery }));
    };
        ...
        <div>
            <div className="bg-surface rounded-card shadow-card p-4 md:p-6 mb-6">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    {/* Search Input - First */}
                    <div className="flex-3 md:min-w-[400px]">
                        <input
                            type="text"
                            placeholder="Search by name, phone, or email..."
                            onChange={handleSearchChange}
                            className="w-full px-4 py-2 border border-border rounded-button focus:ring-2 focus:ring-primary focus:border-primary bg-background text-text-primary placeholder-text-secondary"
                        />
                    </div>
                    ...
                    <select
                        ...
                        className="px-4 py-2 border border-border rounded-button focus:ring-2 focus:ring-primary focus:border-primary bg-background text-text-primary md:w-[180px]"
                    >
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap">
                                                    {booking.status === 'cancelled' ? (
                                                        <select
                                                            ...
                                                            disabled
                                                            className="text-xs font-semibold rounded-badge px-2 py-1 border border-border bg-gray-100 text-gray-500 cursor-not-allowed"
                                                        >
                                                            <option value="cancelled">Cancelled</option>
                                                        </select>
                                                    ) : (
