...
function CalendarView({ isUserView = false, coachId = null, staffName = null }) {
    ...
    const eventStyleGetter = (event) => {
        ...
        if (event.is_special_event) {
            ...
            // Legacy coaching — per-coach colour
            const coachColors = [
                                        ...
                                        <div className="absolute bottom-full left-0 mb-3 hidden group-hover:block z-[100] bg-surface-light border border-border rounded-xl shadow-2xl p-4 min-w-[240px] backdrop-blur-md bg-opacity-95">
                                            ...
                                            <div className="space-y-3">
                                                {coachList.length > 0 ? coachList.map(coach => (
                                                    <div key={coach.id} className="flex items-center transition-transform hover:translate-x-1">
                                                        <div className="w-4 h-4 rounded-full mr-3 shadow-inner border border-white/10" style={{ backgroundColor: coachColors[coach.id % coachColors.length] }} />
                                                        <div className="text-sm font-medium text-text-primary">{coach.first_name} {coach.last_name}</div>
                                                    </div>
                                                )) : (
